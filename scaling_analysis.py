...
ax1.plot(range(N), systemiterators[1].avalanche_sizes_scaled)
ax1.set_xlabel('t')
...
for i, s in enumerate(systemiterators):
    ax2.plot(range(N), s.total_height_hist, label=L[i])
    ax2.set_xlabel('t')
    ax2.set_ylabel('h')
    ax2.legend(loc='best')
    
    ax3.plot(range(N), s.total_height_hist_scaled, label=L[i])
    ax3.set_xlabel('t')
    ax3.set_ylabel('h/L')
    ax3.legend(loc='best')
